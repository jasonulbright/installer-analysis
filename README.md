...
![Installer Analysis - dark theme](screenshots/installer-analysis-dark.png)
*Overview tab showing a PostgreSQL BitRock installer analyzed end-to-end:
type detection, silent switches, deployment fields, file metadata, signature
validation, and the MECM-ready export action bar. Log drawer captures each
analysis pass; status bar reflects the resolved display name.*

![Installer Analysis - light theme](screenshots/installer-analysis-light.png)
*Same view on Light.Blue. Every brush is theme-adaptive; WCAG AA on both
themes is verified via the repo's brand contrast harness.*

...
| `Get-InstallerType`, `Get-VelopackMetadata` | Detect the setup bundle signature, read its offset/length-delimited nuspec ZIP, and derive silent commands, default install location, HKCU ARP key and short DisplayVersion | [velopack/velopack](https://github.com/velopack/velopack) — `src/bins/src/setup.rs` (bundle header and CLI), `src/lib-rust/src/bundle.rs` (manifest), `src/lib-rust/src/locator.rs` (short version), `src/bins/src/commands/install.rs` (default folder), `src/bins/src/windows/registry.rs` (ARP values) | MIT | Vendor (Velopack project) |
